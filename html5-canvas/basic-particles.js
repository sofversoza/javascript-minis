// Create blue particles on screen and make each of them move in diff directions
...
// canvas resize with the browser & distorts it (stretches our drawing), so we set a resize listener to make sure everytime the window changes canvas is sync (only setting lines 5-6 isnt enough bc it'll only run once on page render)
window.addEventListener("resize", function(){
  canvas.width = window.innerWidth;     
  canvas.height = window.innerHeight; 
})

// to get the users x & y coordinates so we can apply an event listener & draw on the canvas depending on the coordinates
const mouse = {
  x: undefined,
  y: undefined,
}
